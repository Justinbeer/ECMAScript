...
function testFilter() {
    console.log("=========== filter");
    // filter : 내부 요소는 그대로, 조건 만족하는 요소만 뽑아서 새 배열생성
...
function testReduce() {
    ...
    let sum = source.reduce((acc, value, index, arr) => {
        console.log(
            ...
            // acc -> 현재까지의 집계 값
            // value -> 현재 값
            return acc + value; //  -> 다음 번 콜백의 acc로 전환
...
function testDataPipeline() {
    console.log("========== map, filter, sort, reduce를 " + "이용한 데이터 파이프라인");
    console.log("원본데이터 : ", data);

    // map 함수 이용 -> total 파생 변수
    const studentsWithTotal = data.map(s => ({...s,
        total : s.kor + s.eng + s.math
    }));
    console.log("map : ", studentsWithTotal);

    // filter 함수 이용 -> total >= 240만 출력
    const filteredStudents = studentsWithTotal.filter(
